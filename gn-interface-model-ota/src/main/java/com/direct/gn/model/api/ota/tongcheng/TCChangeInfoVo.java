 ...
 * ChangeInfo Element
 */
public class TCChangeInfoVo implements Serializable {
	/**
	 * 
	 */
	private static final long serialVersionUID = 8627055055165049576L;
	
	// 改期类型0：客票全部未使用；1：客票部分使用（1 仅往返程可使用）
	private Integer changeType;
	// 改期标识 T：不可改期 H：有条件改期 F：免费改期 E：改签规则以航空公司为准；
	private String changeStatus;
	// 改期金额，当 changeStatus=H 时，必须赋值；如果 changeStatus =T/F,则此字段可不赋值
	private Integer changeFee;
	// 改期费币种，当 changesStatus =H，必须赋值。 IATA 标准币种编码, （目前仅限和 Routing 报价币种一致）
	private String currency;
	// 乘客类型，0 成人 /1 儿童
	private Integer passengerType;
	// 是否允许 NoShow 改期，T：不可改，E：按航司客规为准，H：有条件改，F：免费改
	private String chaNoshow;
	// NoShow 时限，即起飞前多久算 NoShow；单位：小时 ；如不赋值则认为航班起飞时间算 NoShow 时间节点
	private Integer chaNoShowCondition;
	// NoShow 后改期费用，即算上 NoShow 罚金后的改期费用；当 chaNoshow =H，必须赋值
